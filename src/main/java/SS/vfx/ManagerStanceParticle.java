...
/**
 * ManagerStance 能力粒子：脚底向上翻飞的金色光点，旋转上升、微光闪烁渐隐。
 * 贴图使用内置 GLOW_SPARK 光斑（小而不挡视野）。
 * 与 FiendStanceParticle 的唯一区别：alpha 叠加正弦微光闪烁，而非纯平滑渐隐。
 */
public class ManagerStanceParticle extends AbstractGameEffect {
    private float x;
    private float y;
    private float baseX;
    private float velY;
    ...
    // 微光闪烁参数
    private float shimmerFreq;
    private float shimmerPhase;

    private static final Color GOLD = new Color(1.0f, 0.85f, 0.2f, 1f);

    public ManagerStanceParticle(AbstractCreature owner) {
        ...
        this.shimmerFreq = MathUtils.random(8f, 14f);
        this.shimmerPhase = MathUtils.random(0f, 6.283f);
    }
    ...
    public void update() {
        ...
        // 透明度：开头 10% 快速淡入，55% 之后平滑渐隐，全程叠加正弦微光闪烁
        float alpha = 0.7f;
        if (t < 0.1f) {
            alpha *= t / 0.1f;
        }
        ...
        float shimmer = 0.65f + 0.35f * MathUtils.sin(elapsed * this.shimmerFreq + this.shimmerPhase);
        this.color.a = Math.max(0f, alpha * shimmer);
    }
